...
public class ChampsTaxiFender extends SequentialCommandGroup{
    public ChampsTaxiFender(DoubleSupplier shootStall, DoubleSupplier driveStall, Drivetrain drivetrain, Intake intake, LowerIndex lowerIndex, UpperIndex upperIndex, Flywheel flywheel, Wrist wrist){
        super(
            new TimedFlywheel(flywheel, Flywheel.State.FULL, shootStall.getAsDouble()),
            new ParallelCommandGroup(
                new TimedShootCargo(upperIndex, lowerIndex, .5),
                new TimedFlywheel(flywheel, Flywheel.State.FULL, .5)),
            new WaitCommand(driveStall.getAsDouble()),
            new DriveStraight(drivetrain, 2.3, true)
            );
    }
}
